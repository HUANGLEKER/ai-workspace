...
## 子项目文档（先看对应子目录的 AGENTS.md）

本仓库为三服务单体仓库，**改某一层前请先读该层的 `AGENTS.md`**，本根文件只保留跨服务的架构、契约与约定：

| 子项目 | 目录 | 文档 | 职责 |
|---|---|---|---|
| 前端 | `ai-workspace-web/` | [AGENTS.md](ai-workspace-web/AGENTS.md) | Vue3 + TS + Tailwind 4 + Radix Vue |
| Go 后端 | `ai-workspace-backend/` | [AGENTS.md](ai-workspace-backend/AGENTS.md) | 鉴权 / RBAC / 业务逻辑 / 资源归属 |
| AI 服务 | `ai-service/` | [AGENTS.md](ai-service/AGENTS.md) | 所有 LLM 交互（流式/嵌入/RAG/agent/工作流） |
...
> 各服务的完整命令、配置项、规范见对应子目录 `AGENTS.md`。下表为最小启动序列。
...
用户私有资源按当前用户隔离。归属列名不一致：KB/文档/Agent/Workflow/Prompt/Tool/MCP→`create_by`；Chat/RAG 会话/消息→`user_id`；文件→`upload_by`。**唯一强制入口**是 `internal/service/owned.go` 的泛型 helper（`getOwnedResource[T]` 单条 404/403 三态、`ownedScope[T]` 列表过滤）；**禁止手写 `Where("create_by = ?")` 字面量**。详见后端 AGENTS.md。

## RBAC / 鉴权

JWT Claims：`userID`、`username`、`roles`（带 `ROLE_` 前缀）。中间件注入 `gin.Context`，`AdminRequired()` 校验 `ROLE_ADMIN`。前端从 `/auth/info` 的 roles 暴露 `isAdmin`，隐藏系统菜单、路由守卫加载用户信息。`chat_model.api_key` AES-256-GCM 加密落库；CORS 白名单、限流分级、强制改密详见后端 AGENTS.md。
...
- **workflow-runner**: 在 Codex / OpenClaw / Cursor 中直接运行 agency-orchestrator YAML 工作流
