...
func (s *jobService) PageJobs(pageNum, pageSize int, jobName string) (common.PageResult[model.SysJob], error) {
	...
	var total int64
	q := database.DB.Model(&model.SysJob{})
	if jobName != "" {
...
func (s *jobService) DeleteJob(id int64) error {
	...
	return database.DB.Unscoped().Delete(&model.SysJob{}, id).Error
}
...
func (s *jobService) ChangeStatus(id int64, status int8) error {
	...
	if err = database.DB.Model(&model.SysJob{}).Where("id = ?", id).Update("status", status).Error; err != nil {
		return err
	}
...
func (s *jobService) PageJobLogs(pageNum, pageSize int, jobID int64) (common.PageResult[model.SysJobLog], error) {
	...
	var total int64
	q := database.DB.Model(&model.SysJobLog{})
	if jobID > 0 {
...
func (s *jobService) CleanLogs() error {
	return database.DB.Where("1 = 1").Delete(&model.SysJobLog{}).Error
}
...
func (s *jobService) LoadAndScheduleRunning() error {
	var jobs []model.SysJob
	if err := database.DB.Where("status = ?", model.JobStatusRunning).Find(&jobs).Error; err != nil {
		return err
	}
