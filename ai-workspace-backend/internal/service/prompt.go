...
// PromptService 依赖经构造函数注入（P1-1）
type PromptService struct {
	db *gorm.DB
}

// NewPromptService 构造服务
...
func (s *PromptService) ListByUser(userID int64, keyword, category string) ([]model.Prompt, error) {
	...
	q := s.db.Scopes(ownedScope[model.Prompt](userID))
	if keyword != "" {
...
func (s *PromptService) GetOwned(id, userID int64) (*model.Prompt, error) {
	return getOwnedResource[model.Prompt](s.db, id, userID, "提示词")
}
...
func (s *PromptService) Delete(id, userID int64) error {
	...
	return s.db.Delete(&model.Prompt{}, id).Error
}
