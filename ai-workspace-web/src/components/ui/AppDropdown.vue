    ...
    <DropdownMenuPortal>
      ...
      >
        <template v-for="(item, idx) in items" :key="item.key">
          ...
          <DropdownMenuItem
            class="flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2 text-sm outline-none transition-all duration-200 ease-out data-[highlighted]:bg-zinc-100/50 dark:data-[highlighted]:bg-zinc-800"
            :class="item.danger ? 'text-red-500' : 'text-zinc-800 dark:text-zinc-100'"
            @select="$emit('select', item.key)"
          >
            <component :is="item.icon" v-if="item.icon" class="h-4 w-4" :class="item.danger ? '' : 'text-zinc-500 dark:text-zinc-400'" />
            {{ item.label }}
          </DropdownMenuItem>
        </template>
      </DropdownMenuContent>
